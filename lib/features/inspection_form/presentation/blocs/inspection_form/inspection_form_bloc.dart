...
class InspectionFormBloc extends Bloc<InspectionFormEvent, InspectionFormState> {
  ...
  InspectionFormBloc({required SubmitInspectionForm submitInspectionForm})
    ...
    on<InspectionFormEvent>((event, emit) => emit(InspectionFormLoadingState()));

    on<InspectionFormSubmitEvent>((event, emit) async {
      await Future.delayed(const Duration(seconds: 1));
      final res = await _submitInspectionForm(event.data);
      res.fold((l) => emit(InspectionFormFailureState(l.message)), (r) => emit(InspectionFormSubmittedState(data: event.data)));
    });
  }
}
