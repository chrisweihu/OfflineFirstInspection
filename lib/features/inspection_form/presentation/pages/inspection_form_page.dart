...
class InspectionFormPage extends StatefulWidget {
  ...
  static MaterialPageRoute<InspectionFormPage> route({required InspectionFormDto formData, required String mode}) => MaterialPageRoute(
  ...
  const InspectionFormPage({super.key, required InspectionFormDto formData, required String mode}) : _formData = formData, _mode = mode;
...
class _InspectionFormPageState extends State<InspectionFormPage> {
  ...
  Scaffold buildPageScaffold(InspectionFormState state, BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('${state is InspectionFormSubmittedState ? 'View' : _initialMode} Inspection'),
        actions: [
            ...
            ElevatedButton.icon(
              onPressed: () {
                ...
                if (_formKey.currentState?.validate() ?? false) {
                  ...
                  context.read<InspectionFormBloc>().add(InspectionFormSubmitEvent(data: _formData));
                }
              },
      ...
      body: SingleChildScrollView(
        child: Padding(
          ...
          child: AbsorbPointer(
            absorbing: (state is InspectionFormSubmittedState || _initialMode == 'View') ? true : false,
            child: Form(
              key: _formKey,
              child: Column(
                spacing: 10,
                ...
                children: [
                  InspectionTextField(
                    onSaved: (val) => _formData.inspector = val ?? _formData.inspector,
                  ...
                  InspectionDropdownField(
                    onSaved: (val) => _formData.status = val ?? _formData.status,
                  ...
                  InspectionTextField(
                    onSaved: (val) => _formData.summary = val ?? _formData.summary,
                  ...
                  InspectionCheckboxField(
                    onSaved: (val) {
                      _formData.reviewRequired = val ?? _formData.reviewRequired;
                    },
                  ...
                  BlocBuilder<FormCheckboxCubit, FormCheckboxState>(
                    ...
                    buildWhen: (previousState, state) {
                      return (state is FormCheckboxUpdatedState && state.fieldId == InspectionFormPage.fidReviewRequired);
                    },
                    builder: (context, state) {
                      return (state is FormCheckboxUpdatedState && state.value)
                          ? InspectionTextField(
                              ...
                              onSaved: (val) {
                                _formData.reviewDescription = val ?? _formData.reviewDescription;
                              },
                  ...
                  InspectionCheckboxField(
                    onSaved: (val) {
                      _formData.actionRequired = val ?? _formData.actionRequired;
                    },
                  ...
                  BlocBuilder<FormCheckboxCubit, FormCheckboxState>(
                    // return true/false to determine whether or not to rebuild the widget with state
                    buildWhen: (previousState, state) {
                      return (state is FormCheckboxUpdatedState && state.fieldId == InspectionFormPage.fidActionRequired);
                    },
                    builder: (context, state) {
                      return (state is FormCheckboxUpdatedState && state.value)
                          ? InspectionTextField(
                              ...
                              onSaved: (val) {
                                _formData.actionDescription = val ?? _formData.actionDescription;
                              },
